...
function init() {
    ...
    document.getElementById('login-role').addEventListener('change', (e) => {
        ...
        if (role === 'customer') {
            ...
            document.getElementById('login-email').value = 'tech1@akfeek.com';
        }
    });
}
...
async function createBooking() {
    try {
        ...
        log('Sending Towing Request...', 'info');
        // Correct endpoint for Towing Request
        const res = await axios.post(`${API_URL}/bookings/towing/request`, bookingData);
...
function trackBooking(bookingId) {
    ...
    const pollInterval = setInterval(async () => {
        try {
            // Use broadcastId
            const res = await axios.get(`${API_URL}/bookings/towing/${activeBroadcastId}/offers`);
...
async function acceptOffer(broadcastId, offerId) {
    try {
        ...
        await axios.post(`${API_URL}/bookings/towing/${broadcastId}/offers/${offerId}/accept`);
...
async function loadTechnicianJobs() {
    try {
        ...
        const res = await axios.get(`${API_URL}/technician/towing/jobs`);
...
async function loadBroadcasts() {
    try {
        ...
        const res = await axios.get(`${API_URL}/technician/towing/broadcasts`);
...
async function submitOffer(broadcastId) {
    try {
        ...
        await axios.post(`${API_URL}/technician/towing/broadcasts/${broadcastId}/offer`, offerData);
