  ...
  try {
    ...
    if (row.targetType === 'SERVICE') {
      ...
      if (service) {
        ...
        const offerPrices = priceByVehicleType.map((x) => x.offerPrice).filter((n) => Number.isFinite(n));
        const currentPrices = priceByVehicleType.map((x) => x.currentPrice).filter((n) => Number.isFinite(n));
        const wasPrices = priceByVehicleType.map((x) => x.wasPrice).filter((n) => Number.isFinite(n));
    ...
    } else if (row.targetType === 'CERTIFIED_WORKSHOP_SERVICE') {
    ...
    } else if (row.targetType === 'MOBILE_WORKSHOP_SERVICE') {
